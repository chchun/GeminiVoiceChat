...
- **음성 대화**: 마이크 버튼을 누르면 전체 화면 오버레이가 떠서 사용자의 말을 듣고, 생각하고, 음성으로 답합니다 (Gemini Live와 동일한 UX).

### 개발자 입장
- 현재는 가짜 AI(Mock)가 동작합니다. 실제 서버 없이 UI/UX의 완성도를 검증하기 위함입니다.
- 나중에 진짜 Gemini API 또는 자체 WebSocket 서버로 교체할 때, **UI/ViewModel 코드는 한 줄도 안 바꿔도 되도록** 구조가 짜여 있습니다(이 책임 분리를 가능하게 하는 원칙이 DIP — 의존성 역전 원칙).
...
2. 좌하단 **마이크 버튼** 탭 → 마이크 권한 허용 → 보이스 오버레이가 뜨면서 말을 시작하세요.
...
│   │   └── VoiceOverlay.kt       # 보이스 모드 전체화면 오버레이 Composable
│   └── theme/
│       ├── Color.kt              # 색상 팔레트
...
[ChatScreen.onMicClicked]
   ...
   │                  → 허용 시 viewModel.startVoiceMode()
   │                  → 거부 시 Toast 표시
   ▼
[ChatViewModel.startVoiceMode]
   │ state.voice.isActive = true, mode = LISTENING
   │ collectVoiceEvents()                   ← Flow<VoiceEvent> 구독 시작
   │ voiceController.startListening()
   ▼
[AndroidVoiceController.startListening]
   │ Main 스레드에서:
   │   ├─ createOnDeviceSpeechRecognizer() ← Bixby 우회 (API 31+)
   │   ├─ EXTRA_LANGUAGE = "ko-KR"
   │   ├─ EXTRA_PREFER_OFFLINE = true
   │   └─ recognizer.startListening(intent)
   ▼
[RecognitionListener 콜백들]
   │ onReadyForSpeech → VoiceEvent.ReadyForSpeech emit
   │ onRmsChanged → VoiceEvent.Rms(데시벨) emit (초당 수십번)
   │   → ViewModel이 amplitude(0..1) 정규화 → 오브 크기 애니메이션
   │ onPartialResults → VoiceEvent.Partial(부분 텍스트) emit
   │ onEndOfSpeech → VoiceEvent.EndOfSpeech emit
   │   → ViewModel이 mode = THINKING 으로 전환 (오브 잔잔한 펄스)
   │ onResults → VoiceEvent.Final(최종 텍스트) emit
   ▼
[ChatViewModel.onSpeechRecognized]
   │ submitUserPrompt(text, speakResult=true)
   │   → 텍스트 채팅과 동일 경로로 USER 메시지 + AI placeholder 추가
   │   → AI 응답 스트리밍 완료 후:
   ▼
[ChatViewModel.submitUserPrompt 끝부분]
   │ if (speakResult) voiceController.speak(fullText)
   ▼
[AndroidVoiceController.speak]
   │ tts.speak(text, QUEUE_FLUSH, ...)
   ▼
[UtteranceProgressListener 콜백]
   │ onStart → VoiceEvent.SpeakingStarted → mode = SPEAKING (빠른 펄스)
   │ onDone → VoiceEvent.SpeakingFinished
   │   → ViewModel.resumeListeningIfActive()
   │   → mode = LISTENING, voiceController.startListening() 재시작
   ▼
[순환: 사용자가 X 버튼을 누를 때까지 LISTENING ↔ THINKING ↔ SPEAKING 반복]
```
...
### 시나리오: 실제 백엔드(WebSocket 또는 Gemini API) 가 준비되었을 때

**Step 1.** `data/repository/RemoteAiRepository.kt` 새 파일 생성:

```kotlin
class RemoteAiRepository(
    private val client: OkHttpClient  // 또는 어떤 네트워크 라이브러리든
) : AiRepository {
    override suspend fun sendMessage(text: String): Flow<String> = flow {
        // 실제 WebSocket 또는 HTTP 호출로 chunk를 emit
    }.flowOn(Dispatchers.IO)
...
**Step 2.** `di/AppContainer.kt`의 단 한 줄을 수정:
...
override val aiRepository: AiRepository by lazy {
    RemoteAiRepository(provideOkHttpClient())
}
```

**Step 3.** 끝.

- `ChatViewModel`, `ChatScreen`, `ChatState`는 손대지 않습니다.
- 보이스 모드도 그대로 동작합니다 (STT/TTS는 여전히 로컬, AI 호출만 서버로).
- 테스트는 `MockAiRepository`로 계속 실행 가능.
...
- **[TODO.md](TODO.md)** — 미구현 항목 (배지인, 문장단위 TTS, RemoteAiRepository 스캐폴딩 등).
